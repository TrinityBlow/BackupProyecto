<?php
	...
	require_once($_SERVER['DOCUMENT_ROOT'] . '/View/EliminarPacientes.php');
		...
		if((strpos($parametros, 'page=') === 0)){
			if (($autenticacion['login']) == true){
				...
				if($page < ResourceController::getInstance()->getPageLimitPacientes()){
					$autenticacion['sigPage'] = 'eliminarPacientes.php?page='.(string)($page + 1);
				} else{
					$autenticacion['sigPage'] = 'eliminarPacientes.php?page='.(string)($page);
				}
				if($page > 1){
					$autenticacion['prevPage'] = 'eliminarPacientes.php?page='.(string)($page - 1);
				} else{
					$autenticacion['prevPage'] = 'eliminarPacientes.php?page=1';
				}
				ResourceController::getInstance()->showPaginadoPacientes('EliminarPacientes',$page,$autenticacion);
			}
		} else{
