...
class ResourceRepository extends PDORepository {
	...
	public function loginForm($username,$password){
        $answer = $this->queryList("SELECT * FROM users where username=? and password=?",array($username,$password));
		return $answer;

    }

	public function signupForm($username,$password){
        $answer = $this->queryInsert("INSERT INTO users (username, password) VALUES (?, ?)",array($username,$password));
		return $answer;
